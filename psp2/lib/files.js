        ...
        fs.readFile(fp, {
            ...
            for (var i = 0; i < xs.length; i++) {
                if (_this.isNumeric(xs[i]) && _this.isNumeric(ys[i])) {
                    var simp = (Number(xs[i]) / Number(ys[i]));
                    response.push(simp);
                } else {
                    error = true;
                }
            ...
            if (error) {
                return callback(new MyError('El archivo está mal escrito, o tiene numeros negativos'));
            }
...
files.prototype.toJSON = function(obj) {
    var newSizes = [];
    for(var property in obj.nr){
      newSizes.push(obj.nr[property]);
    }
    obj.sizes = newSizes;
